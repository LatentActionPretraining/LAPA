...
class FlaxVideoLLaMAModule(nn.Module):
    ...
    def __call__(
        ...
        if input_ids.shape[1] == 1:
            ...
            # print("vision_masks shape", vision_masks.shape, input_ids.shape)
            # if vision_masks.shape[1] != input_ids.shape[1]:
            #     vision_masks = vision_masks[:,-input_ids.shape[1]:]
            # print("edited vision_masks", vision_masks)
...
class FlaxVideoLLaMAForCausalLM(FlaxVideoLLaMAPreTrainedModel):
    ...
    def _sample_vision(
        ...
        def sample_search_body_fn(state):
            ...
            print("model logits", model_outputs.logits.shape)
            ...
            next_token = jax.lax.cond(
                ...
                lambda: jnp.full_like(next_token, 8193),
                lambda: next_token
            )
        ...
        # return FlaxSampleOutput(sequences=state.sequences), state.model_kwargs['past_key_values']
    ...
    def generate_vision(
        ...
        **kwargs,
        ...
        self.config.sample_mode = 'vision'
